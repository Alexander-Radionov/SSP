...
function parseBankAccount(bankAccount) {
    var Arrtmp = bankAccount.split('\n');
    var accNum = 0;
    var numIfDig = 9;
    for (var i = 0; i < numIfDig; i++){
        var tmpd;
        var k = i*3;
        if (Arrtmp[2][k+2] === ' '){
            tmpd = 2;
...
function getPokerHandRank(hand) {
    ...
    var signSame = isSignSame(cards);
    var straight = isStraight(cards);
    var freq = frequencyCounting(cards);
...
function isStraight(cards){
    var values = cards.map((x)=>x[0]).filter(x=>x !== 0); //cut out A because of double behaviors
    values.sort((x,y)=>x > y);
...
function* getFigureRectangles(figure) {
    ...
    for (var i = 0; i < Arrtmp.length; i++)
        for (var j = 0; j < Arrtmp[0].length; j++)
            if (Arrtmp[i][j] === '+') {
                pluses.push({x: j, y: i});
            }

    for (var i = 0; i < pluses.length; i++)
        for (var j = i + 1; j < pluses.length; j++)
...
function checkRectangle(Arrtmp, top, bottom) {
    ...
    for (var j = leftX + 1; j < rightX; j++)
        if (Arrtmp[topY][j] === '+' && Arrtmp[bottomY][j] === '+') {
            var hasWhiteSpace = false;

            for (var i = topY + 1; i < bottomY; i++)
                if (Arrtmp[i][j] === ' ')
                    hasWhiteSpace = true;
...
function drawRectangle(item) {
    var width = item[0][1].x - item[0][0].x + 1,

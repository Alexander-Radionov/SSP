...
function getPolynom() {
    ...
    return (x) => {
        return arr.reduce((prev, curr, index) => prev+curr*Math.pow(x,index),0);
    };
    throw new Error('Not implemented');
}
...
function partialUsingArguments(fn) {
    ...
    tmp.splice(0,1);
    return function (){
        tmp = tmp.concat(Array.from(arguments));

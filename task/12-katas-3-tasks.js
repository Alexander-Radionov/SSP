...
function findStringInSnakingPuzzle(puzzle, searchStr) {
    function dfs(current, step) {
        var save = data[current.y][current.x];
    ...
    var data = Array.from(puzzle);
    var Arrtmp = Array.from({length: data[0].length + 2}, () => '');
...
encode: function(url) {
        ...
        for (var i = 0; i < url.length; i += 2) {
            var tmp1 = url.charCodeAt(i);
            var tmp2 = url.charCodeAt(i + 1);
            var code = (tmp1 << 8) |tmp2;
    ...
    decode: function(code) {
        ...
        for (var i = 0; i < code.length; i++) {
            var char = parseInt(code.charCodeAt(i), 10);
            var tmp1 = char & 255;
            var tmp2 = (char >> 8) & 255;
            if (tmp1 === 0) {

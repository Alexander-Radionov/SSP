...
function createCompassPoints() {
    function* directCoors(from, to, dir) {
        var res = [`${from}b${to}`, `${from}${from}${to}`, `${from}${to}b${from}`, `${from}${to}`, `${from}${to}b${to}`, `${to}${from}${to}`, `${to}b${from}`];
...
function getZigZagMatrix(n) {
    var res = new Array(n).fill().map(()=> new Array(n).fill());
    var i = 0, j = 0;
    var d = -1;
